...
4. **Print the plan, then proceed** — V#, slug, target Dropbox path, what folders you'll create. Paul has standing permission (granted 11/07/2026) for Dropbox file/folder creation — do NOT wait for confirmation to create. Only pause if the project or V# allocation is genuinely ambiguous.
...
- **Standing permission (Paul, 11/07/2026): create, save, move and copy files/folders in Dropbox WITHOUT asking.** Do the filing, report what was done afterwards. The two exceptions that still require Paul's explicit go-ahead: (1) **deleting** anything, (2) overwriting/superseding an existing record — never do either without checking. Never delete old records; add new versions or addendum files alongside.
- **Ask Paul only for substantive variation judgment calls** — ambiguous project/V# allocation, scope interpretation, cost allocation between variations, same-V#-or-new-V# decisions, and anything affecting what the client is charged. Scrutinise the source material, give feedback, and flag discrepancies — but don't block routine filing on approvals.
- **Register updates**: generate the updated register, deliver it via SendUserFile, and summarise the new/changed rows in chat — no pre-approval needed. When preparing costs, assess ALL available documents (emails, screenshots, PDFs, invoices, receipts) before finalising figures.
